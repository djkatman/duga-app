...
@php
  ...
  // レビュー/評価（星）
  $ratingValue = null;
  if (isset($rating) && is_numeric($rating)) $ratingValue = max(0, min(5, (int)$rating));
  $reviewCount = (is_numeric($mylistTotal) ? (int)$mylistTotal : null); // なければ null のまま
  ...
  $productLd = [
    ...
    // レーティング（あるときだけ）
    'aggregateRating'=> (!is_null($ratingValue)) ? [
...
@php
  $firstCat = (!empty($categories) && isset($categories[0]) && method_exists($categories[0],'getName')) ? $categories[0] : null;
  $crumbs = [
    ['label' => 'トップ', 'url' => route('home')],
  ];
  if ($firstCat && method_exists($firstCat,'getId')) {
    $crumbs[] = [
      'label' => 'カテゴリ: '.$firstCat->getName(),
      'url'   => route('browse.filter',['type'=>'category','id'=>$firstCat->getId()]),
    ];
  }
 ...
 <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="p-3 text-xs text-gray-500">
          @if($jSmall || $jMedium || $jLarge)
            <div class="mt-1">パッケージ:
              @if($jSmall)<a class="underline" href="{{ $jSmall }}" target="_blank">S</a>@endif
              @if($jMedium)<span class="mx-1">/</span><a class="underline" href="{{ $jMedium }}" target="_blank">M</a>@endif
              @if($jLarge)<span class="mx-1">/</span><a class="underline" href="{{ $jLarge }}" target="_blank">L</a>@endif
            </div>
          @endif
        </div>
      </div>
      ...
      @endif
        ...
        <div class="bg-white rounded-lg shadow p-3">
            ...
            @foreach($thumbs as $idx => $t)
                @php
                // ライトボックス用に URL を変換
                $fullUrl = str_replace('/noauth/scap/', '/cap/', $t);
                @endphp
                <button
                type="button"
    ...
    <div class="md:col-span-2 space-y-6">
      <div class="bg-white rounded-lg shadow p-4">
        ...
        <div class="mt-4 flex flex-wrap gap-3 text-lg">
          @if($url)
            <!-- <a href="{{ $url }}" target="_blank" class="inline-flex items-center gap-1 px-3 py-1 rounded border hover:bg-gray-50">公式ページ</a> -->
          @endif
          @if($affUrl)
  ...
  <span>公式ページで詳細を見る</span>
    ...
    : ($has($item,'getRating') ? $item->getRating() : null);
          ...
          $reviewer = method_exists($review,'getReviewer') ? $review->getReviewer() : null;
...
@endif
      ...
      {{-- 開発支援：Rawダンプ（必要な時だけ使ってください） --}}
      <details class="bg-white rounded-lg shadow p-4">
        <summary class="cursor-pointer text-sm text-gray-600">開発用：オブジェクトの中身を表示</summary>
        <pre class="mt-3 text-xs overflow-auto max-h-[40vh]">{{ print_r($item, true) }}</pre>
      </details>
    </div>
  </div>
  ...
  <div id="lightboxContent" class="relative z-10 max-w-5xl w-full">
    ...
    <div class="relative bg-black rounded-lg shadow overflow-hidden">
      <img id="lightboxImg"
           src=""
           alt="sample"
           class="mx-auto max-h-[80vh] w-auto select-none" draggable="false" />
      {{-- 閉じるボタン --}}
      <button id="lbClose"
      ...
      @foreach($related as $it)
        ...
        @if($pid)
          <a href="{{ route('products.show', ['id' => $pid]) }}"
            ...
            <div class="p-2">
              <div class="line-clamp-2 text-xs font-medium leading-snug">{{ $title }}</div>
            </div>
          </a>
        @endif
      @endforeach
...
<script>
  (function () {
    ...
    function loadImage(idx) {
      ...
      preloadNext.src = sources[(idx + 1) % total];
      const preloadPrev = new Image();
      preloadPrev.src = sources[(idx - 1 + total) % total];
    }
